...
class BERTTrainer:
    ...
    def iteration(self, epoch, max_batches, data_loader, train=True, split_name='', evalu=False):
        ...
        data_iter = tqdm.tqdm(enumerate(data_loader), desc="%s" % (split_name), total=len(data_loader),
                              bar_format="{l_bar}{r_bar}")
        for i, data in data_iter:
            if i == max_batches: break

            task_inputs = (data["bert_input"][0].to(self.device), data["bert_input"][1].to(self.device),
                           data["bert_input"][2].to(self.device))
