...
class TransformerClassifier(nn.Module):
    def __init__(self, hidden, n_classes, dropout=0.1, seq_len=512):
        ...
        self.transformer_blocks = nn.ModuleList([nn.TransformerEncoderLayer(hidden, attn_heads,
                                                                            dim_feedforward=hidden * 4, dropout=dropout,
                                                                            activation=activation, eps=1e-12) for _ in
                                                 range(n_layers)])
...
class conv_base_model(nn.Module):
    ...
    def forward(self, iput):
        ...
        padded_iput = torch.cat(([torch.cat(([F.pad(ibatch.unsqueeze(0), (
        0, 0, self.padding - j, self.kernelsize - self.padding - 1 + j + self.poolingsize)) for j in
                                              range(self.poolingsize)]), dim=0) for ibatch in iput]), dim=0)
