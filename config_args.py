...
def get_args(parser, eval=False):
    ...
    parser.add_argument("--emb_type", type=str, default='conv',
                        choices=['lookup', 'conv', 'continuous', 'both', 'pair'])
    ...
    if args.debug:
        ...
        args.train_dataset = os.path.join(args.data_root,
                                          args.train_dataset) if args.train_dataset is not None else None
        args.valid_dataset = os.path.join(args.data_root,
                                          args.valid_dataset) if args.valid_dataset is not None else None
