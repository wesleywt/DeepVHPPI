...
class JsonDatasetPairwiseClassification(Dataset):
    ...
    def __init__(self, corpus_path, vocab, esm, esm_alphabet, esm_batch_converter, seq_len, corpus_lines=None,
                 random_subseq=False):
        self.vocab = vocab
        self.esm = esm
...
def load_data_single(args, vocab, esm_alphabet, esm_batch_converter):
    """Load method for each of the datasets
    ...
    if args.task == 'secondary':
        ...
        train_dataset = JsonDatasetTagging(args.train_dataset, vocab, args.seq_len, 'ss3',
                                           corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetTagging(args.valid_dataset, vocab, args.seq_len,
                                           'ss3') if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetTagging(args.test_dataset, vocab, args.seq_len,
                                          'ss3') if args.test_dataset is not None else None

    if args.task == 'transmembrane':
        num_classes = 4
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetTransmembrane(args.train_dataset, vocab, args.seq_len,
                                                 corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetTransmembrane(args.valid_dataset, vocab,
                                                 args.seq_len) if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetTransmembrane(args.test_dataset, vocab,
                                                args.seq_len) if args.test_dataset is not None else None

    elif args.task == 'contact':
        num_classes = 1
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetContact(args.train_dataset, vocab, args.seq_len,
                                           corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetContact(args.valid_dataset, vocab,
                                           args.seq_len) if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetContact(args.test_dataset, vocab,
                                          args.seq_len) if args.test_dataset is not None else None
        ...
        num_classes = 1195
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetClassification(args.train_dataset, vocab, args.seq_len, 'fold_label',
                                                  corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetClassification(args.valid_dataset, vocab, args.seq_len,
                                                  'fold_label') if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetClassification(args.test_dataset, vocab, args.seq_len,
                                                 'fold_label') if args.test_dataset is not None else None

    elif args.task == 'fluorescence':
        num_classes = 1
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetRegression(args.train_dataset, vocab, args.seq_len, 'log_fluorescence',
                                              corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetRegression(args.valid_dataset, vocab, args.seq_len,
                                              'log_fluorescence') if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetRegression(args.test_dataset, vocab, args.seq_len,
                                             'log_fluorescence') if args.test_dataset is not None else None


    elif args.task == 'stability':
        num_classes = 1
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetRegression(args.train_dataset, vocab, args.seq_len, 'stability',
                                              corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetRegression(args.valid_dataset, vocab, args.seq_len,
                                              'stability') if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetRegression(args.test_dataset, vocab, args.seq_len,
                                             'stability') if args.test_dataset is not None else None


    elif args.task == 'solubility':
        num_classes = 2
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetClassification(args.train_dataset, vocab, args.seq_len, 'solubility',
                                                  corpus_lines=args.corpus_lines,
                                                  evo=True) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetClassification(args.valid_dataset, vocab, args.seq_len, 'solubility',
                                                  evo=True) if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetClassification(args.test_dataset, vocab, args.seq_len, 'solubility',
                                                 evo=True) if args.test_dataset is not None else None


    elif args.task == 'localization':
        num_classes = 24
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetClassification(args.train_dataset, vocab, args.seq_len, 'localization',
                                                  corpus_lines=args.corpus_lines,
                                                  evo=True) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetClassification(args.valid_dataset, vocab, args.seq_len, 'localization',
                                                  evo=True) if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetClassification(args.test_dataset, vocab, args.seq_len, 'localization',
                                                 evo=True) if args.test_dataset is not None else None

    elif args.task == '4prot':
        num_classes = 3
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDataset4Prot(args.train_dataset, vocab, seq_len=args.seq_len,
                                         corpus_lines=args.corpus_lines) if args.train_dataset is not None else None
        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDataset4Prot(args.valid_dataset, vocab,
                                         seq_len=args.seq_len) if args.valid_dataset is not None else None
        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDataset4Prot(args.test_dataset, vocab,
        ...
        num_classes = 1
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetPairwiseClassification(args.train_dataset, vocab, args.esm, esm_alphabet,
                                                          esm_batch_converter, seq_len=args.seq_len,
                                                          corpus_lines=args.corpus_lines,
                                                          random_subseq=True) if args.train_dataset is not None else None
        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetPairwiseClassification(args.valid_dataset, vocab, args.esm, esm_alphabet,
                                                          esm_batch_converter, seq_len=args.seq_len2,
                                                          corpus_lines=args.corpus_lines) if args.valid_dataset is not None else None
        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetPairwiseClassification(args.test_dataset, vocab, args.esm, esm_alphabet,
                                                         esm_batch_converter, seq_len=args.seq_len2,
                                                         corpus_lines=args.corpus_lines) if args.test_dataset is not None else None

    elif args.task == 'malaria':
        num_classes = 2
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetTagging(args.train_dataset, vocab, args.seq_len, 'label',
                                           corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetTagging(args.valid_dataset, vocab, args.seq_len,
                                           'label') if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetTagging(args.test_dataset, vocab, args.seq_len,
                                          'label') if args.test_dataset is not None else None

    elif args.task == 'lm':
        num_classes = len(vocab) - 3
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = JsonDatasetLM(args.train_dataset, vocab, seq_len=args.seq_len,
                                      corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = JsonDatasetLM(args.valid_dataset, vocab, seq_len=args.seq_len,
                                      corpus_lines=args.corpus_lines) if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = JsonDatasetLM(args.test_dataset, vocab, seq_len=args.seq_len,
                                     corpus_lines=args.corpus_lines) if args.test_dataset is not None else None

    elif args.task == 'test_lm':
        num_classes = len(vocab) - 3
        print("Loading Train Dataset", args.train_dataset)
        train_dataset = Starr2020Dataset(args.train_dataset, vocab, seq_len=args.seq_len,
                                         corpus_lines=args.corpus_lines) if args.train_dataset is not None else None

        print("Loading Valid Dataset", args.valid_dataset)
        valid_dataset = Starr2020Dataset(args.valid_dataset, vocab, seq_len=args.seq_len,
                                         corpus_lines=args.corpus_lines) if args.valid_dataset is not None else None

        print("Loading Test Dataset", args.test_dataset)
        test_dataset = Starr2020Dataset(args.test_dataset, vocab, seq_len=args.seq_len,
                                        corpus_lines=args.corpus_lines) if args.test_dataset is not None else None

    print("Creating Dataloader")
    train_data_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True,
                                   collate_fn=train_dataset.collate_fn,
                                   num_workers=args.num_workers) if train_dataset is not None else None

    valid_data_loader = DataLoader(valid_dataset, batch_size=args.batch_size, shuffle=False,
                                   collate_fn=valid_dataset.collate_fn,
                                   num_workers=args.num_workers) if valid_dataset is not None else None

    test_data_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False,
                                  collate_fn=test_dataset.collate_fn,
                                  num_workers=args.num_workers) if test_dataset is not None else None
...
def get_data_loader(task, train_dataset, valid_dataset, test_dataset, num_classes, args):
    ...
    """
    train_data_loader = DataLoader(train_dataset, args.batch_size, shuffle=True, collate_fn=train_dataset.collate_fn,
                                   num_workers=args.num_workers)
    valid_data_loader = DataLoader(valid_dataset, args.batch_size, shuffle=False, collate_fn=valid_dataset.collate_fn,
                                   num_workers=args.num_workers) if valid_dataset is not None else None
    test_data_loader = DataLoader(test_dataset, args.batch_size, shuffle=False, collate_fn=test_dataset.collate_fn,
                                  num_workers=args.num_workers) if test_dataset is not None else None
...
def load_data_multi(args, vocab):
    ...
    num_classes = 3
    task = 'secondary'
    train_dataset = JsonDatasetTagging('data/data/secondary/train.json', vocab, args.seq_len, 'ss3',
                                       corpus_lines=args.corpus_lines)
    valid_dataset = JsonDatasetTagging('data/data/secondary/valid.json', vocab, args.seq_len, 'ss3',
                                       corpus_lines=args.corpus_lines)
    test_dataset = JsonDatasetTagging('data/data/secondary/cb513.json', vocab, args.seq_len, 'ss3',
                                      corpus_lines=args.corpus_lines)
    data.append(get_data_loader(task, train_dataset, valid_dataset, test_dataset, num_classes, args))

    num_classes = 1
    task = 'contact'
    train_dataset = JsonDatasetContact('data/data/contact/test.json', vocab, args.seq_len,
                                       corpus_lines=args.corpus_lines)
    valid_dataset = JsonDatasetContact('data/data/contact/test.json', vocab, args.seq_len,
                                       corpus_lines=args.corpus_lines)
    test_dataset = JsonDatasetContact('data/data/contact/test.json', vocab, args.seq_len,
                                      corpus_lines=args.corpus_lines)
    data.append(get_data_loader(task, train_dataset, valid_dataset, test_dataset, num_classes, args))

    num_classes = 1195
    task = 'homology'
    train_dataset = JsonDatasetClassification('data/data/homology/train.json', vocab, args.seq_len, 'fold_label',
                                              corpus_lines=args.corpus_lines, evo=args.use_evo)
    valid_dataset = JsonDatasetClassification('data/data/homology/valid.json', vocab, args.seq_len, 'fold_label',
                                              corpus_lines=args.corpus_lines, evo=args.use_evo)
    test_dataset = JsonDatasetClassification('data/data/homology/test.json', vocab, args.seq_len, 'fold_label',
                                             corpus_lines=args.corpus_lines, evo=args.use_evo)
    data.append(get_data_loader(task, train_dataset, valid_dataset, test_dataset, num_classes, args))
